...
# GCN based model
class GCNNet_PLIG_with_p(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.2, protein_processing_type="convolution", featurizer=False):
        ...
        super(GCNNet_PLIG_with_p, self).__init__()
        ...
        self.dropout_connection_layers = 0.017
        self.act = "leaky_relu"
        ...
        self.gnn_1 = GCNConv(num_features_xd, num_features_xd * 2 ** self.output_dim_power_0)
        self.gnn_2 = GCNConv(num_features_xd * 2 ** self.output_dim_power_0, num_features_xd * 2 ** self.output_dim_power_1)
        self.gnn_3 = GCNConv(num_features_xd * 2 ** self.output_dim_power_1, num_features_xd * 2 ** self.output_dim_power_2)
        self.gnn_4 = GCNConv(num_features_xd * 2 ** self.output_dim_power_2,
                             num_features_xd * 2 ** self.output_dim_power_3)
        self.gnn_5 = GCNConv(num_features_xd * 2 ** self.output_dim_power_3,
                             num_features_xd * 2 ** self.output_dim_power_4)
        self.fc_g1 = nn.Linear(num_features_xd * 2 ** self.output_dim_power_4, self.output_dim)
