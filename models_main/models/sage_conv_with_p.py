...
# SAGEConv based model
class SageNet_with_p(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.2, protein_processing_type="convolution",  featurizer=False):
        ...
        super(SageNet_with_p, self).__init__()
        self.featurizer = featurizer
        self.output_dim = output_dim
        self.protein_processing_type = protein_processing_type
        # hyperparameter opt learning rate: 0.0003
        self.n_filters_power = 5
        self.embed_dim = embed_dim
        self.kernel_size_power = 5
        self.dropout_GNN_layers = 0.000
        self.output_dim_power_0 = 1
        self.output_dim_power_1 = 0
        self.output_dim_power_2 = 1
        self.output_dim_power_3 = 0
        self.output_dim_power_4 = 2
        self.output_dim_power_5 = 2
        self.dropout_connection_layers = 0.016
        self.act = "leaky_relu"
        self.activation = activation_function_dict[self.act]

        self.gnn_1 = SAGEConv(num_features_xd, num_features_xd * 2 ** self.output_dim_power_0)
        self.gnn_2 = SAGEConv(num_features_xd * 2 ** self.output_dim_power_0, num_features_xd * 2 ** self.output_dim_power_1)
        self.gnn_3 = SAGEConv(num_features_xd * 2 ** self.output_dim_power_1,
                              num_features_xd * 2 ** self.output_dim_power_2)
        self.gnn_4 = SAGEConv(num_features_xd * 2 ** self.output_dim_power_2,
                              num_features_xd * 2 ** self.output_dim_power_3)
        self.gnn_5 = SAGEConv(num_features_xd * 2 ** self.output_dim_power_3,
                              num_features_xd * 2 ** self.output_dim_power_4)
        self.gnn_6 = SAGEConv(num_features_xd * 2 ** self.output_dim_power_4,
                              num_features_xd * 2 ** self.output_dim_power_5)

        self.fc_g1 = nn.Linear(num_features_xd * 2 ** self.output_dim_power_5, self.output_dim)
    ...
    def forward(self, data):
        ...
        x = self.gnn_5(x, edge_index)
        x = self.activation(x)
        x = self.dropout_gnn(x)

        x = self.gnn_6(x, edge_index)
        x = self.activation(x)
        x = self.dropout_gnn(x)
