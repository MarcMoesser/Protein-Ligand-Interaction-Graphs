...
# GINConv model
class GINConvNet_no_p(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.2, protein_processing_type="convolution", featurizer=False):
        ...
        super(GINConvNet_no_p, self).__init__()
        ...
        self.gnn_1 = GINConv(Sequential(Linear(num_features_xd, num_features_xd * 2 ** self.output_dim_power_0),
                ...
                eps=self.eps)
        self.gnn_2 = GINConv(Sequential(Linear(num_features_xd * 2 ** self.output_dim_power_0, num_features_xd * 2 ** self.output_dim_power_1),
                                        self.activation_mlp,
                                        Linear(num_features_xd * 2 ** self.output_dim_power_1,
                                               num_features_xd * 2 ** self.output_dim_power_1)),
                             eps=self.eps)

        self.gnn_3 = GINConv(Sequential(
            Linear(num_features_xd * 2 ** self.output_dim_power_1, num_features_xd * 2 ** self.output_dim_power_2),
            self.activation_mlp,
            Linear(num_features_xd * 2 ** self.output_dim_power_2,
