...
# GCN based model
class GCNNet_with_p(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.2, protein_processing_type="convolution", featurizer=False):
        ...
        super(GCNNet_with_p, self).__init__()
        ...
        self.gnn_4 = GCNConv(num_features_xd * 2 ** self.output_dim_power_2, num_features_xd * 2 ** self.output_dim_power_3)
        self.fc_g1 = nn.Linear(num_features_xd * 2 ** self.output_dim_power_3, self.output_dim)
