...
class GAT_GCN_PLIG_no_p(torch.nn.Module):
    def __init__(self, n_output=1, num_features_xd=78, num_features_xt=25,
        ...
        self.dropout_connection_layers = 0.073
        self.act = "leaky_relu"
        ...
        self.gnn2_1 = GCNConv(num_features_xd * 2 ** self.output_dim_power_0 * self.heads_1,
                              num_features_xd * 2 ** self.output_dim_power_1)
        self.gnn2_2 = GATConv(num_features_xd * 2 ** self.output_dim_power_1,
                              num_features_xd * 2 ** self.output_dim_power_1, heads=self.heads_2)

        self.fc_g1 = nn.Linear(num_features_xd * 2 ** self.output_dim_power_1 * self.heads_2, output_dim)
    ...
    def forward(self, data):
        ...
        x = self.gnn2_1(x, edge_index)
        x = self.activation(x)
        x = self.dropout_gnn(x)

        x = self.gnn2_2(x, edge_index)
        x = self.activation(x)
        x = self.dropout_gnn(x)
